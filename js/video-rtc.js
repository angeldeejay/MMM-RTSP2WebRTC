 ...
 * Adapted for MagicMirror² (removed ES module export).
 *
 * Support:
...
class VideoRTC extends HTMLElement {
    constructor() {
        ...
        /** [config] Supported modes (webrtc, webrtc/tcp, mse, hls, mp4, mjpeg). @type {string} */
        this.mode = 'webrtc,mse,hls,mjpeg';

        /** [config] Requested medias (video, audio, microphone). @type {string} */
        this.media = 'video,audio';

        /** [config] Run stream when not displayed on the screen. @type {boolean} */
        this.background = false;

        /** [config] Visibility threshold 0–1. 0 = disabled. @type {number} */
        this.visibilityThreshold = 0;

        /** [config] Pause when browser tab hidden. @type {boolean} */
        this.visibilityCheck = true;

        /** [config] WebRTC configuration. @type {RTCConfiguration} */
        this.pcConfig = {
        ...
        /** [info] WebSocket state. @type {number} */
        this.wsState = WebSocket.CLOSED;

        /** [info] WebRTC state. @type {number} */
    ...
    play() {
        this.video.play().catch(() => {
            if (!this.video.muted) {
                ...
                this.video.play().catch(er => console.warn(er));
            }
        });
    }
    ...
    onwebrtc() {
        ...
        pc.addEventListener('connectionstatechange', () => {
            if (pc.connectionState === 'connected') {
                ...
                this.pcState = WebSocket.CLOSED;
                this.pc = null;
                // Close WS to guarantee onclose() fires and the reconnect timer runs,
                // regardless of whether the server closes the socket on its own.
                if (this.ws) {
                    this.ws.close();
                    this.ws = null;
                }
